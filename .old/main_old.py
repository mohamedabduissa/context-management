...
class RelevanceScorer:
    def get_top_k_messages(self, history: List[Message], token_budget: int) -> List[Message]:
        ...
        for msg in reversed(history):
            msg_tokens = get_token_count(msg.content or "")
...
class ContextBudgetManager:
    ...
    def build_payload(self, session_id: str, system_prompt: str, user_query: str, recent_tools: List[Message]) -> tuple[List[Message], dict]:
        ...
        raw_tokens = get_token_count(system_prompt) + get_token_count(user_query)
        raw_tokens += sum(get_token_count(m.content or "") for m in history)
        raw_tokens += sum(get_token_count(m.content or "") for m in recent_tools)
        ...
        remaining_budget = (
            ...
            - get_token_count(system_prompt)
            - get_token_count(ctx_str)
            - get_token_count(user_query)
        )
        
        # 4. Tools
        tools_selected = 0
        for tool in recent_tools:
            t_tokens = get_token_count(tool.content or "")
        ...
        tokens_after = sum(get_token_count(m.content or "") for m in final_payload)
...
async def openai_chat_endpoint(req: Request, background_tasks: BackgroundTasks):
    ...
    original_token_count = sum([get_token_count(str(m.get("content", "")) + str(m.get("tool_calls", ""))) for m in messages])
    data["messages"] = final_messages
    logger.info(f"[CONTEXT DEBUG] Incoming tokens: ~{original_token_count} | Compressed tokens sent to vLLM: ~{current_tokens} | Budget Limit: {budget}")
    
    # 2. Transparent Streaming Proxy
    if stream:
        async def async_generator():
            try:
                async with async_http_client.stream("POST", "http://localhost:8000/v1/chat/completions", json=data) as resp:
                    resp.raise_for_status()
        ...
        try:
            resp = await async_http_client.post("http://localhost:8000/v1/chat/completions", json=data)
